...
export function IndonesiaMap({ projects, onSelectProject }: { projects: DbProject[]; onSelectProject: (p: DbProject) => void }) {
  ...
  return (
    <div className="glass-card rounded-lg p-4 animate-slide-up shadow-card">
      ...
      <div className="relative w-full aspect-[2.2/1] rounded-lg overflow-hidden border border-border bg-gradient-to-b from-primary/5 to-primary/10">
        <svg viewBox="0 0 1000 450" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M80,120 L120,100 L160,110 L200,130 L230,160 L260,200 L280,240 L290,280 L270,300 L240,290 L210,260 L180,230 L150,200 L120,170 L90,150 Z"
            fill="hsl(215, 30%, 85%)" stroke="hsl(215, 40%, 65%)" strokeWidth="1" />
          {/* Java */}
          ...
          <path d="M560,120 L580,100 L600,110 L610,140 L600,170 L610,200 L630,210 L640,240 L630,260 L610,250 L600,220 L590,200 L580,180 L570,160 L560,140 Z"
            fill="hsl(215, 30%, 85%)" stroke="hsl(215, 40%, 65%)" strokeWidth="1" />
          {/* NTT chain */}
          <path d="M510,310 L525,305 L530,312 L520,315 Z" fill="hsl(215, 30%, 85%)" stroke="hsl(215, 40%, 65%)" strokeWidth="0.8" />
          <path d="M540,315 L560,310 L570,316 L555,320 Z" fill="hsl(215, 30%, 85%)" stroke="hsl(215, 40%, 65%)" strokeWidth="0.8" />
          <path d="M580,320 L600,315 L615,320 L600,328 Z" fill="hsl(215, 30%, 85%)" stroke="hsl(215, 40%, 65%)" strokeWidth="0.8" />
          <path d="M625,325 L650,318 L665,325 L650,335 Z" fill="hsl(215, 30%, 85%)" stroke="hsl(215, 40%, 65%)" strokeWidth="0.8" />
          {/* Maluku */}
          <path d="M700,160 L720,150 L730,170 L720,190 L700,185 Z" fill="hsl(215, 30%, 85%)" stroke="hsl(215, 40%, 65%)" strokeWidth="0.8" />
          <path d="M690,220 L710,210 L720,230 L710,250 L690,240 Z" fill="hsl(215, 30%, 85%)" stroke="hsl(215, 40%, 65%)" strokeWidth="0.8" />
          {/* Papua */}
          ...
          {[100, 200, 300, 400].map((y) => (
            <line key={`h${y}`} x1="0" y1={y} x2="1000" y2={y} stroke="hsl(215, 20%, 90%)" strokeWidth="0.3" strokeDasharray="4,8" />
          ))}
          {[200, 400, 600, 800].map((x) => (
            <line key={`v${x}`} x1={x} y1="0" x2={x} y2="450" stroke="hsl(215, 20%, 90%)" strokeWidth="0.3" strokeDasharray="4,8" />
          ))}
        </svg>
        ...
        {projects.map((project) => (
          ...
          >
            ...
            <span className={`relative block w-2.5 h-2.5 rounded-full ${statusColors[project.status]} border-2 border-card shadow-md transition-transform group-hover:scale-[2]`} />
