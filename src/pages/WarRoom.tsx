    ...
    <div className="h-screen flex flex-col text-white overflow-hidden" style={{ background: "linear-gradient(180deg, hsl(220,25%,6%) 0%, hsl(220,20%,10%) 100%)" }}>
      <header className="flex items-center justify-between px-5 py-2.5 border-b flex-shrink-0" style={{ borderColor: "hsl(220, 20%, 14%)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold tracking-tight leading-tight">Dashboard Control Tower</h1>
          ...
          <div className="flex gap-2 min-h-0" style={{ height: "45%" }}>
            <div className="flex-1 rounded-xl border p-3" style={{ background: "hsl(220, 20%, 9%)", borderColor: "hsl(220, 20%, 16%)" }}>
              ...
              <div className="h-[calc(100%-24px)]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={pieData} cx="50%" cy="50%" innerRadius="35%" outerRadius="65%" paddingAngle={3} dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={false} fontSize={9} fill="hsl(215, 80%, 55%)">
              ...
              <div className="flex-1 rounded-xl border overflow-hidden relative" style={{ background: "hsl(220, 20%, 9%)", borderColor: "hsl(220, 20%, 16%)" }}>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3">
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <span className="text-[9px] font-mono-data px-1.5 py-0.5 rounded-full" style={{ background: "hsl(215,80%,55%,0.2)", color: "hsl(215,80%,65%)" }}>{featured.project_code}</span>
                    <span className="text-[9px] px-1.5 py-0.5 rounded-full font-medium" style={{ background: "hsl(152,55%,50%,0.2)", color: "hsl(152,55%,60%)" }}>{featured.progress}%</span>
                  </div>
                  <h4 className="text-xs font-bold text-white leading-tight line-clamp-2">{featured.name}</h4>
                  <p className="text-[9px] mt-0.5 flex items-center gap-1" style={{ color: "hsl(215,15%,50%)" }}><MapPin className="h-2.5 w-2.5" />{featured.location}</p>
                  <div className="mt-1.5 w-full h-1.5 rounded-full overflow-hidden" style={{ background: "hsl(220,20%,20%)" }}>
                    <div className="h-full rounded-full" style={{ width: `${featured.progress}%`, background: `linear-gradient(90deg, hsl(215,80%,55%), hsl(152,55%,50%))` }} />
                  </div>
                </div>
                {active.length > 1 && (
                  <div className="absolute top-1 right-1 flex gap-0.5">
                    <button onClick={() => setFeaturedIdx(i => (i - 1 + active.length) % active.length)} className="p-1 rounded-full hover:bg-white/20 transition-colors" style={{ background: "hsl(0,0%,0%,0.3)" }}><ChevronLeft className="h-3 w-3 text-white" /></button>
                    <button onClick={() => setFeaturedIdx(i => (i + 1) % active.length)} className="p-1 rounded-full hover:bg-white/20 transition-colors" style={{ background: "hsl(0,0%,0%,0.3)" }}><ChevronRight className="h-3 w-3 text-white" /></button>
                  </div>
                )}
              </div>
            )}
